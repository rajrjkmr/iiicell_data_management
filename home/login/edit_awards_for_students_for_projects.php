...
$userid=$_SESSION['myValue'];
 ...
 $sql="UPDATE awards_for_students_for_projects SET NAME_OF_THE_ORGANISATION='$company_name',PERSON_CONTACTED='$person_contacted',CONTACT_NUMBER='$contact_number',CURRENT_STATUS='$outcome',dept='$dept',userid='$userid' WHERE id='$id'";
if(mysqli_query($conn, $sql)){  
 header("Location:AWARDS_FOR_STUDENTS_FOR_PROJECTS.php");
}else{  
...
$sql="SELECT *FROM awards_for_students_for_projects where ID='$id'";
 $result_set=mysqli_query($conn, $sql);
 while($row=mysqli_fetch_array($result_set))
 {
 	$company_name=$row['NAME_OF_THE_ORGANISATION'];
 	$person_contacted=$row['PERSON_CONTACTED'];
 	$contact_number=$row['CONTACT_NUMBER'];
 	$outcome=$row['CURRENT_STATUS'];
...
<center><fieldset>
<label ><span>NAME OF THE ORGANISATION: <span class="required"></span></span><input type="text" class="input-field" name="company_name" value="<?php echo "$company_name"; ?>" /></label>
...
<label for="field1"><span>CURRENT STATUS: <span class="required"></span></span><input type="text" class="input-field" name="outcome" value="<?php echo "$outcome"; ?>" /></label>
