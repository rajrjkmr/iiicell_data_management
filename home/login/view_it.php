...
$sql = "SELECT * FROM implant_training  WHERE company_name='$drop'";  
...
echo '<tr>';
 echo '<td>'.$row['company_name'].'</td><td>'.nl2br($row['address']).'</td><td>'.$row['email'].'</td><td>'.$row['phone'].'</td><td>'.$row['attend_by'].'</td><td>'.$row['form_date'].'</td><td>'.$row['to_date'].'</td><td>'.$row['sumbit_date'].'</td><td>'.$row['message'].'</td><td>'.$row['userid'].'</td>';?><td><a href="delete_it.php?id=<?php echo $id; ?>" 
    onclick="return confirm('Are you sure you wish to delete this Record?');">
            <span class="delete" title="Delete"> X </span></a></td><td><a href="edit_it.php?id=<?php echo $id; ?>"><span class="edit" title="Edit"> E </span></a></td> </table><?php
echo '<tr>';
...
<table  border="5px" style="width:120%;height: 50%; margin-left:; color:red;font-weight:bold;font-size:15pt;">
...
$sql = "SELECT * FROM implant_training  WHERE company_name='$drop' AND dept='$dept'";  
$retval=mysqli_query($conn, $sql);  
...
echo '<tr>';
 echo '<td>'.$row['company_name'].'</td><td>'.nl2br($row['address']).'</td><td>'.$row['email'].'</td><td>'.$row['phone'].'</td><td>'.$row['attend_by'].'</td><td>'.$row['form_date'].'</td><td>'.$row['to_date'].'</td><td>'.$row['sumbit_date'].'</td><td>'.$row['message'].'</td>';?><td><a href="delete_it.php?id=<?php echo $id; ?>" 
    onclick="return confirm('Are you sure you wish to delete this Record?');">
            <span class="delete" title="Delete"> X </span></a></td><td><a href="edit_it.php?id=<?php echo $id; ?>"><span class="edit" title="Edit"> E </span></a></td> </table><?php
echo '<tr>';
