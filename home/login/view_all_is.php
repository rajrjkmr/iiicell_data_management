...
$sql = "SELECT * FROM indernship WHERE dept='$dept'";  
$retval=mysqli_query($conn, $sql);  
...
{  
 ...
 while($row = mysqli_fetch_assoc($retval))
	 { 
	 	$id=$row['id'];
 echo '<tr>';
 echo '<td>'.$row['company_name'].'</td><td>'.$row['project_name'].'</td><td>'.nl2br($row['address']).'</td><td>'.$row['email'].'</td><td>'.$row['phone'].'</td><td>'.$row['attend_by'].'</td><td>'.$row['form_date'].'</td><td>'.$row['to_date'].'</td><td>'.$row['sumbit_date'].'</td><td>'.$row['message'].'</td>';?><td><a href="delete_is_all.php?id=<?php echo $id; ?>" 
    onclick="return confirm('Are you sure you wish to delete this Record?');">
            <span class="delete" title="Delete"> X </span></a></td><td><a href="edit_is.php?id=<?php echo $id; ?>"><span class="edit" title="Edit"> E </span></a></td><?php
echo '<tr>';
