...
<html>
<center><h2 style="color: #d81b60;padding-left: 4em;font-size:24pt;font-family:Forte;">INFORMATION RETRIEVE INPLANT TRAINING</h2></center>
<style type="text/css">
th,td{color: #8e24aa;
...
<?php
 while($row = mysqli_fetch_assoc($retval))
	 { 
	 	 $id=$row['id'];
 echo '<tr>';
 echo '<td>'.$row['COMPANY_NAME'].'</td><td>'.$row['PERSON_CONTACTED'].'</td><td>'.$row['CONTACT_NUMBER'].'</td><td>'.$row['CURRENT_STATUS'].'</td><td>'.$row['submit_date'].'</td><td>'.$row['userid'].'</td>';?><td><a href="delete_ainternship.php?id=<?php echo $id; ?>" 
    onclick="return confirm('Are you sure you wish to delete this Record?');">
  	          <span class="delete" title="Delete"> X </span></a></td><td><a href="edit_ainternship.php?id=<?php echo $id; ?>"><span class="edit" title="Edit"> E </span></a></td><!-- <td>
...
$sql = "SELECT *FROM ainternship WHERE dept='$dept'";  
$retval=mysqli_query($conn, $sql);  
...
{  
 ...
 while($row = mysqli_fetch_assoc($retval))
	 { 
	 	$id=$row['id'];
 echo '<tr>';
 echo '<td>'.$row['COMPANY_NAME'].'</td><td>'.$row['PERSON_CONTACTED'].'</td><td>'.$row['CONTACT_NUMBER'].'</td><td>'.$row['CURRENT_STATUS'].'</td><td>'.$row['submit_date'].'</td>';?><td><a href="delete_ainternship.php?id=<?php echo $id; ?>" 
    onclick="return confirm('Are you sure you wish to delete this Record?');">
            <span class="delete" title="Delete"> X </span></a></td><td><a href="edit_ainternship.php?id=<?php echo $id; ?>"><span class="edit" title="Edit"> E </span></a></td><?php
echo '<tr>';
