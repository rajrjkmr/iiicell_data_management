...
<table   border="5px" style=" color:red;width: 180%;height: 50%;font-weight:bold;font-size:15pt;">
...
$sql = "SELECT * FROM indernship WHERE company_name='$drop' ";  
...
{  
 ...
 while($row = mysqli_fetch_assoc($retval))
	 { 
	 	$id=$row['id'];
  echo '<tr>';
 echo '<td>'.$row['company_name'].'</td><td>'.$row['project_name'].'</td><td>'.nl2br($row['address']).'</td><td>'.$row['email'].'</td><td>'.$row['phone'].'</td><td>'.$row['attend_by'].'</td><td>'.$row['form_date'].'</td><td>'.$row['to_date'].'</td><td>'.$row['sumbit_date'].'</td><td>'.$row['userid'].'</td><td>'.$row['message'].'</td>';?><td><a href="delete_is.php?id=<?php echo $id; ?>" 
    onclick="return confirm('Are you sure you wish to delete this Record?');">
  	          <span class="delete" title="Delete"> X </span></a></td><td><a href="edit_is.php?id=<?php echo $id; ?>"><span class="edit" title="Edit"> E </span></a></td> </table><?php
echo '<tr>';
...
<table   border="5px" style=" width: 180%;height: 40%;color:red;font-weight:bold;font-size:15pt;">
<tr><th>NAME OF COMPANY</th> <th>PROJECT NAME</th> <th>ADDRESS</th> <th>EMAIL</th> <th>PHONE</th><th>TRAINING ATTENDED BY</th><th> DATE FROM</th> <th>DATE TO</th> <th>SUBMIT DATE&TIME</th><th>MESSAGE</th> <th>DELETE</th><th>EDIT</th> </tr>  
...
$sql = "SELECT * FROM indernship WHERE company_name='$drop' AND dept='$dept'";  
$retval=mysqli_query($conn, $sql);  
...
{  
 ...
 while($row = mysqli_fetch_assoc($retval))
	 { 
	 	$id=$row['id'];
 echo '<tr>';
 echo '<td>'.$row['company_name'].'</td><td>'.$row['project_name'].'</td><td>'.nl2br($row['address']).'</td><td>'.$row['email'].'</td><td>'.$row['phone'].'</td><td>'.$row['attend_by'].'</td><td>'.$row['form_date'].'</td><td>'.$row['to_date'].'</td><td>'.$row['sumbit_date'].'</td><td>'.$row['message'].'</td>';?><td><a href="delete_is.php?id=<?php echo $id; ?>" 
    onclick="return confirm('Are you sure you wish to delete this Record?');">
            <span class="delete" title="Delete"> X </span></a></td><td><a href="edit_is.php?id=<?php echo $id; ?>"><span class="edit" title="Edit"> E </span></a></td> </table><?php
echo '<tr>';
