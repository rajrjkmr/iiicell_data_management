...
<table   border="5px" style=" color:red;font-weight:bold;font-size:15pt;width: 180%;height: 50%;">
...
$sql = "SELECT * FROM industry_visit WHERE company_name='$drop' ";  
...
{  
 ...
 echo '<tr>';
 echo '<td>'.$row['company_name'].'</td><td>'.nl2br($row['address']).'</td><td>'.$row['email'].'</td><td>'.$row['phone'].'</td><td>'.$row['attend_by'].'</td><td>'.$row['date'].'</td><td>'.$row['submit_date'].'</td><td>'.$row['userid'].'</td>' ?><td><a href="delete_iv.php?id=<?php echo $id; ?>" 
...
$sql = "SELECT * FROM industry_visit WHERE company_name='$drop' AND dept='$dept'";  
$retval=mysqli_query($conn, $sql);  
...
{  
 ...
 while($row = mysqli_fetch_assoc($retval))
	 { 
	 	$id=$row['id'];
 echo '<tr>';
 echo '<td>'.$row['company_name'].'</td><td>'.nl2br($row['address']).'</td><td>'.$row['email'].'</td><td>'.$row['phone'].'</td><td>'.$row['attend_by'].'</td><td>'.$row['date'].'</td><td>'.$row['submit_date'].'</td>' ?><td><a href="delete_iv.php?id=<?php echo $id; ?>" 
    onclick="return confirm('Are you sure you wish to delete this Record?');">
            <span class="delete" title="Delete"> X </span></a></td> <td><a href="edit_iv.php?id=<?php echo $id; ?>"><span class="edit" title="Edit"> E </span></a></td>; <?php
echo '<tr>';
