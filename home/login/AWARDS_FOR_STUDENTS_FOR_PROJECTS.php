...
<html>
<center><h2 style="color: #d81b60;padding-left: 4em;font-size:24pt;font-family:Forte;">INFORMATION RETRIEVE AWARDS FOR STUDENTS FOR PROJECTS</h2></center>
<style type="text/css">
th,td{color: #8e24aa;
...
<tr><th>NAME OF THE ORGANISATION</th> <th>PERSON CONTACTED</th> <th>CONTACT NUMBER</th> <th>CURRENT STATUS</th><th>SUBMIT DATE&TIME</th><th>SUBMITED STAFF ID</th><th>DELETE</th><th>EDIT</th><!-- <th>SELECT</th> --></tr>  
<?php  
...
$sql = "SELECT * FROM awards_for_students_for_projects ";  
...
<?php
 while($row = mysqli_fetch_assoc($retval))
	 { 
	 	 $id=$row['id'];
 echo '<tr>';
 echo '<td>'.$row['NAME_OF_THE_ORGANISATION'].'</td><td>'.$row['PERSON_CONTACTED'].'</td><td>'.$row['CONTACT_NUMBER'].'</td><td>'.$row['CURRENT_STATUS'].'</td><td>'.$row['submit_date'].'</td><td>'.$row['userid'].'</td>';?><td><a href="delete_awards_for_students_for_projects.php?id=<?php echo $id; ?>" 
    onclick="return confirm('Are you sure you wish to delete this Record?');">
  	          <span class="delete" title="Delete"> X </span></a></td><td><a href="edit_awards_for_students_for_projects.php?id=<?php echo $id; ?>"><span class="edit" title="Edit"> E </span></a></td><!-- <td>
...
<tr><th>NAME OF THE ORGANISATION</th> <th>PERSON CONTACTED</th> <th>CONTACT NUMBER</th> <th>CURRENT STATUS</th><th>SUBMIT DATE&TIME</th><th>DELETE</th><th>EDIT</th></tr>  
...
$sql = "SELECT *FROM awards_for_students_for_projects WHERE dept='$dept'";  
$retval=mysqli_query($conn, $sql);  
...
{  
 ...
 while($row = mysqli_fetch_assoc($retval))
	 { 
	 	$id=$row['id'];
 echo '<tr>';
 echo '<td>'.$row['NAME_OF_THE_ORGANISATION'].'</td><td>'.$row['PERSON_CONTACTED'].'</td><td>'.$row['CONTACT_NUMBER'].'</td><td>'.$row['CURRENT_STATUS'].'</td><td>'.$row['submit_date'].'</td>';?><td><a href="delete_awards_for_students_for_projects.php?id=<?php echo $id; ?>" 
    onclick="return confirm('Are you sure you wish to delete this Record?');">
            <span class="delete" title="Delete"> X </span></a></td><td><a href="edit_awards_for_students_for_projects.php?id=<?php echo $id; ?>"><span class="edit" title="Edit"> E </span></a></td><?php
echo '<tr>';
