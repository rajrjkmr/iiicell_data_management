...
<html>
<center><h2 style="color: #d81b60;padding-left: 4em;font-size:24pt;font-family:Forte;">INFORMATION RETRIEVE STUDENT PROJECTS IN INDUSTRY</h2></center>
<style type="text/css">
th,td{color: #8e24aa;
...
<tr><th>NAME OF THE COMPANY</th> <th>PERSON CONTACTED</th> <th>CONTACT NUMBER</th> <th>OUTCOME</th><th>SUBMIT DATE&TIME</th><th>SUBMITED STAFF ID</th><th>DELETE</th><th>EDIT</th><!-- <th>SELECT</th> --></tr>  
<?php  
...
$sql = "SELECT * FROM student_projects_in_industry ";  
...
<?php
 while($row = mysqli_fetch_assoc($retval))
	 { 
	 	 $id=$row['id'];
 echo '<tr>';
 echo '<td>'.$row['NAME_OF_THE_COMPANY'].'</td><td>'.$row['PERSON_CONTACTED'].'</td><td>'.$row['CONTACT_NUMBER'].'</td><td>'.$row['OUTCOME'].'</td><td>'.$row['submit_date'].'</td><td>'.$row['userid'].'</td>';?><td><a href="delete_student_projects_in_industry.php?id=<?php echo $id; ?>" 
    onclick="return confirm('Are you sure you wish to delete this Record?');">
  	          <span class="delete" title="Delete"> X </span></a></td><td><a href="edit_student_projects_in_industry.php?id=<?php echo $id; ?>"><span class="edit" title="Edit"> E </span></a></td><!-- <td>
...
<tr><th>NAME OF THE COMPANY</th> <th>PERSON CONTACTED</th> <th>CONTACT NUMBER</th> <th>OUTCOME</th><th>SUBMIT DATE&TIME</th><th>DELETE</th><th>EDIT</th></tr>  
...
$sql = "SELECT *FROM student_projects_in_industry WHERE dept='$dept'";  
$retval=mysqli_query($conn, $sql);  
...
{  
 ...
 while($row = mysqli_fetch_assoc($retval))
	 { 
	 	$id=$row['id'];
 echo '<tr>';
 echo '<td>'.$row['NAME_OF_THE_COMPANY'].'</td><td>'.$row['PERSON_CONTACTED'].'</td><td>'.$row['CONTACT_NUMBER'].'</td><td>'.$row['OUTCOME'].'</td><td>'.$row['submit_date'].'</td>';?><td><a href="delete_student_projects_in_industry.php?id=<?php echo $id; ?>" 
    onclick="return confirm('Are you sure you wish to delete this Record?');">
            <span class="delete" title="Delete"> X </span></a></td><td><a href="edit_student_projects_in_industry.php?id=<?php echo $id; ?>"><span class="edit" title="Edit"> E </span></a></td><?php
echo '<tr>';
